...
public class ViewChangeManager {
    ...
    // max prepared (either max prepared if it exists, or last executed)
    public static int maxSeq(Set<ViewChangeMessage> viewChanges) {
        return viewChanges.stream().map(ViewChangeMessage::maxPrepared).max(Integer::compareTo).orElse(
                viewChanges.stream().map(ViewChangeMessage::getLastExecuted).max(Integer::compareTo).orElse(0)
        );
    }
