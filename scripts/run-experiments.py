...
if __name__ == '__main__':
    for j in range(0, len(ops_sec)):
        for k in range(1, runs+1):
            ...
            subprocess.run(["python3", "scripts/run-open_goods.py", "-c", f"{threads - 1}", "-o", f"{ops_sec[j]}",
    ...
    for i in range(1, 4):
        for j in range(1, runs+1):
            ...
            subprocess.run(f"python3 scripts/run-open_goods.py -c {threads - 1} -o 5 -b 60 -d {duration} "
